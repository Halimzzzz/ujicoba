...
if (isset($_POST['tambah-kriteria'])) {
	...
	$query = mysqli_query($koneksi, "SELECT nama_kriteria FROM kriteria WHERE nama_kriteria = '$nama'");
	if (mysqli_num_rows($query) > 0) {
		header("Location: data-kriteria.php?validasi=warning");
	} else {
		$insert = mysqli_query($koneksi, "INSERT INTO kriteria(nama_kriteria, jenis_kriteria, bobot_kriteria) VALUES('$nama', '$jenis', '$bobot')");
		if ($insert) {
...
if (isset($_POST['edit-kriteria'])) {
	...
	$update = mysqli_query($koneksi, "UPDATE kriteria SET nama_kriteria = '$nama', nama_kriteria = '$nama', jenis_kriteria = '$jenis', bobot_kriteria = '$bobot' WHERE id_kriteria = '$id'");
	if ($update) {
...
if (isset($_POST['tambah-alternatif'])) {
	...
	$query = mysqli_query($koneksi, "SELECT nama_alternatif FROM alternatif WHERE nama_alternatif = '$nama'");
	if (mysqli_num_rows($query) > 0) {
		header("Location: data-alternatif.php?validasi=warning");
	} else {
		$insert = mysqli_query($koneksi, "INSERT INTO alternatif( nama_alternatif, username) VALUES( '$nama' , '$user')");
		if ($insert) {
...
if (isset($_POST['edit-alternatif'])) {
	...
	$update = mysqli_query($koneksi, "UPDATE alternatif SET nama_alternatif = '$nama', username = '$user' WHERE id_alternatif = '$id'");
	if ($update) {
